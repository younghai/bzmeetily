# DMG 제작 현황 (v2.0–v2.0.2 RC6)

이 문서의 `$MEETING_ROOT`는 GitHub용 작업 폴더의 상위 로컬 디렉터리를 뜻한다.

## 2026-09-24: v2.0.2 RC6 창 제어 수정 후보

`v2.0/release/Meetily2_2.0.2-rc.6_aarch64.dmg`를 별도 이미지로 제작했습니다. SHA-256은 `0a54aea9fb5a3cfaeeabdcee8cd6300839bd9bcfa189e1e5dafeb0f0b8274627`입니다. 화면 공유 표시가 macOS 기본 왼쪽 상단 창 버튼을 가릴 수 있어, 앱 안의 닫기·최소화·전체 화면 버튼을 왼쪽 제목 표시줄 아래로 옮겼습니다. 사이드바와 통역 제목에는 버튼이 덮이지 않도록 공간을 확보했습니다. RC5의 겹침 현상을 실기에서 확인하고 RC6에서 수정했습니다. RC6 앱의 일반 창·전체 화면에서 배치를 눈으로 확인했고, 세 버튼의 동작도 확인했습니다. DMG 이미지·체크섬·ad-hoc 서명 검증을 통과했습니다.

RC4는 Mac 잠금이 해제된 뒤 혼합 음성으로 실제 실행했고 일본어/한국어 타임라인 4개가 유지되는 것을 확인했습니다. 다만 조용한 구간에서 잘못된 음성 인식도 발생했습니다. RC6의 음성 정확도를 새로 검증한 것으로 해석하면 안 됩니다. Developer ID 서명·공증, 다른 Mac, 장시간 메모리, 실제 다양한 혼합 오디오 정확도는 여전히 미완료이므로 **일반 배포는 보류**합니다. [RC6 릴리스 노트](release-2.0.2-rc.6.md)를 참고하세요.
