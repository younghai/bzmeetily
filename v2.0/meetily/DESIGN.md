...
macOS can replace the native red/yellow/green traffic lights with its purple screen-sharing indicator while this window is shared. Keep the native title bar. The app also shows a compact, keyboard-labeled close/minimize/full-screen toolbar at the upper right of the webview, clear of the interpretation dialog's own close button. Use the existing white/slate/blue control tokens. This toolbar appears only in the native Tauri app; localhost keeps its browser controls.

## Expanded conversation view — 2026-09-14
The latest user request prefers much larger Japanese-left/Korean-right panes and a longer continuous conversation flow. Interpretation fills the browser viewport without a max-width card or outside gutters. Native startup uses a maximized resizable window. Compact controls leave more vertical space for text. Six consecutive ASR segments form one reading paragraph; their order, Japanese source, Korean translations, errors and pending state remain intact. Each language scrolls independently. New output follows the bottom unless the reader has scrolled up. Chunk-level timing remains in memory; the visual paragraph shows its first timestamp. This changes presentation, not ASR or translation content.
...
Browser live recognition now updates the same utterance while speech arrives, retaining preceding audio in memory until a short pause or bounded window. A small status note explains that captions may be corrected while the speaker continues. Keep the last successful Korean translation visible while its newer revision is pending. Failed translation retains the Japanese text and presents a keyboard-accessible `다시 통역` action next to its error, using existing 44px bordered error-button tokens. No new visual palette/layout is introduced. The earlier fixed two-second browser window description is superseded; meeting recording stays unchanged.

## Conversation timeline — 2026-09-24
The full-screen interpretation view shows a horizontal time-ordered strip above the Japanese and Korean panes. Each entry previews the earlier original and translated text; selecting one scrolls both panes to its conversation paragraph and pauses automatic following. The `최근 대화` action resumes following new captions. A pending translation never removes the original or completed earlier translation. The strip reuses the existing blue, white, and slate tokens and keeps the two reading panes large.
