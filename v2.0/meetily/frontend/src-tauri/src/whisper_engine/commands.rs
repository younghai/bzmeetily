    ...
    if let Some(engine) = engine {
        ...
        match result {
            ...
            Err(e) if is_download_cancelled(&e) => {
                // Report cancellation to the caller: the first-run wizard relies
                // on it to drop its progress state instead of showing a fake
                // 100%. No terminal event; the dedicated cancel command owns
                // the model-list UI state.
