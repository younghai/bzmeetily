    ...
    match store.get("authToken") {
        Some(token) => {
            if let Some(token_str) = token.as_str() {
                // Never log token material, even truncated.
...
pub async fn api_save_transcript<R: Runtime>(
    ...
    // Meeting content must not reach log files; log counts only.
    log_debug!("Processing {} transcript segments", transcripts.len());
