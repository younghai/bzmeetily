...
pub struct SidecarManager {
    ...
    /// Sidecar generation counter: every spawn() bumps it so monitor loops
    /// from older generations exit instead of accumulating forever.
    monitor_epoch: Arc<AtomicU64>,

    /// Active request count (for graceful shutdown)
    active_request_count: Arc<AtomicUsize>,

    /// Serializes whole request/response exchanges. llama-helper speaks a
    /// single-stream JSON-lines protocol without correlation IDs, so two
    /// concurrent generations could otherwise each read the other's response
    /// and store the wrong summary.
...
impl SidecarManager {
    ...
    async fn spawn(&self, model_path: PathBuf) -> Result<()> {
        ...
        // New generation: health/idle loops started below own this epoch;
        // any surviving loops from previous generations retire on next tick.
    ...
    pub async fn send_request(&self, request_json: String, timeout: Duration) -> Result<String> {
        // One exchange at a time (see field docs) so a response can never be
        // delivered to the wrong caller.
        let _serialization = self.request_lock.lock().await;
    ...
    fn start_health_check_loop(&self) {
        let manager = Self {
            ...
            monitor_epoch: self.monitor_epoch.clone(),
        ...
        tokio::spawn(async move {
            let loop_epoch = manager.monitor_epoch.load(Ordering::SeqCst);
            ...
            loop {
                ...
                if manager.monitor_epoch.load(Ordering::SeqCst) != loop_epoch {
                    log::debug!("Health check loop: superseded by newer sidecar generation, exiting");
                    break;
                }
    ...
    fn start_idle_check_loop(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if manager.monitor_epoch.load(Ordering::SeqCst) != loop_epoch {
                    log::debug!("Idle check loop: superseded by newer sidecar generation, exiting");
                    break;
                }
