...
///
#[cfg(target_os = "macos")]
mod screen_capture_sys {
    // CoreGraphics TCC preflight/request for Screen Recording (10.15+).
    // Core Audio process taps are gated behind this permission, so this is
    // the honest check; a denied tap silently captures zeros.
    extern "C" {
...
pub fn check_screen_recording_permission() -> bool {
    // SAFETY: argument-free boolean CoreGraphics queries.
    let granted = unsafe { screen_capture_sys::CGPreflightScreenCaptureAccess() };
    info!("Screen capture (audio tap) permission preflight: {}", granted);
    if !granted {
        info!("If not granted: System Settings → Privacy & Security → Screen Recording");
    }
    granted
}
