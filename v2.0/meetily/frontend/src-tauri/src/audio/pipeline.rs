...
const AUDIO_PIPELINE_CHANNEL_CAPACITY: usize = 256;
...
/// accumulated-speech-buffer warning threshold.
const VAD_REDEMPTION_TIME_MS: u32 = 500;

/// Ring buffer for synchronized audio mixing.
/// A two-source session waits briefly for both windows before emitting one
/// mixed window. Otherwise staggered callbacks duplicate the audio duration.
...
impl AudioMixerRingBuffer {
    ...
    fn can_mix(&self) -> bool {
        ...
        let system = self.system_buffer.len();
        let window = self.window_size_samples;
        match (self.microphone_enabled, self.system_enabled) {
            (true, true) => {
                (mic >= window && system >= window)
                    || mic >= window + window / 2
                    || system >= window + window / 2
            }
            (true, false) => mic >= window,
            (false, true) => system >= window,
            (false, false) => false,
        }
    }
...
pub struct AudioPipeline {
    ...
    /// Optional tap that receives every raw mixed window (before VAD) so a
...
impl AudioPipeline {
    pub fn new(
        ...
        let max_segment_duration_ms = (target_chunk_duration_ms > 0)
            .then_some(target_chunk_duration_ms);
        ...
        let ring_buffer = AudioMixerRingBuffer::new(sample_rate, microphone_enabled, system_enabled);
                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            // STEP 5: Raw mixed tap for the webview live pipeline
                            if let Some(ref sender) = self.raw_mix_sender {
                                let tap_chunk = AudioChunk {
                                    data: mixed_with_gain.clone(),
                                    sample_rate: self.sample_rate,
                                    timestamp: chunk.timestamp,
                                    chunk_id: self.chunk_id_counter,
...
impl AudioPipelineManager {
    ...
    pub fn start(
        ...
        let (audio_sender, audio_receiver) = mpsc::channel::<AudioChunk>(AUDIO_PIPELINE_CHANNEL_CAPACITY);
...
mod tests {
    ...
    #[test]
    fn mixed_audio_waits_for_matching_mic_and_system_windows() {
        let mut buffer = AudioMixerRingBuffer::new(48_000, true, true);
        let window = 28_800;

        buffer.add_samples(DeviceType::Microphone, vec![0.25; window]);
        assert!(!buffer.can_mix(), "a mic-only window must not duplicate mixed output");

        buffer.add_samples(DeviceType::System, vec![0.25; window]);
        assert!(buffer.can_mix());
        let (mic, system) = buffer.extract_window().unwrap();
        assert_eq!((mic.len(), system.len()), (window, window));
        assert!(!buffer.can_mix(), "one pair of input windows yields one mixed window");
    }

    #[test]
    fn single_audio_source_does_not_wait_for_an_absent_device() {
        let mut buffer = AudioMixerRingBuffer::new(48_000, false, true);
        buffer.add_samples(DeviceType::System, vec![0.25; 28_800]);
        assert!(buffer.can_mix());
    }
