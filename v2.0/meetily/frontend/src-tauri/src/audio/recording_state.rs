    ...
    pub fn send_audio_chunk(&self, chunk: AudioChunk) -> Result<()> {
        ...
        if let Some(sender) = self.audio_sender.lock().unwrap().as_ref() {
            sender.try_send(chunk).map_err(|error| match error {
                mpsc::error::TrySendError::Full(_) => anyhow::anyhow!("Audio pipeline channel full"),
                mpsc::error::TrySendError::Closed(_) => anyhow::anyhow!("Audio pipeline channel closed"),
            })?;
...
mod pipeline_channel_tests {
    ...
    #[test]
    fn pipeline_channel_reports_overflow_instead_of_growing_unbounded() {
        // Given: a recording state whose bounded pipeline queue is full.
        let state = RecordingState::new();
        let (sender, _receiver) = mpsc::channel(1);
        state.set_audio_sender(sender);
        state.send_audio_chunk(test_chunk(0)).unwrap();

        // When: the capture callback submits another chunk.
        let error = state.send_audio_chunk(test_chunk(1)).unwrap_err();

        // Then: the overload is explicit and no second chunk is retained.
        assert!(error.to_string().contains("full"));
        assert_eq!(state.get_stats().chunks_processed, 1);
    }

    #[test]
    fn buffer_overflow_is_fatal_to_prevent_silent_audio_loss() {
        // Given: an active recording.
        let state = RecordingState::new();
        state.start_recording().unwrap();

        // When: bounded pipeline capacity is exhausted.
        state.report_error(AudioError::BufferOverflow);

        // Then: recording stops immediately and surfaces the exact error.
        assert!(!state.is_recording());
        assert_eq!(state.get_last_error(), Some(AudioError::BufferOverflow));
    }
}
