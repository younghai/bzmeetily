...
export default function Home() {
  ...
  const sessionBusy = recordingState.isRecording
    || recordingState.isStartingRecording
  ...
  useEffect(() => {
    const openModeChooser = () => {
      if (!sessionBusy) setSessionMode(null);
    };
    sessionStorage.removeItem('autoStartRecording');
    sessionStorage.removeItem('openSessionModeChooser');
    window.addEventListener('open-session-mode-chooser', openModeChooser);
    return () => window.removeEventListener('open-session-mode-chooser', openModeChooser);
  }, [sessionBusy]);

  useEffect(() => {
    if (recordingState.isRecording && sessionMode === null) {
      setSessionMode(recordingState.isLiveInterpretation ? 'interpretation' : 'recording');
    }
  }, [recordingState.isLiveInterpretation, recordingState.isRecording, sessionMode]);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="flex flex-1 overflow-hidden">
        {sessionMode === null ? (
          <main
            className="min-w-0 flex-1 overflow-y-auto bg-slate-50 transition-[margin] duration-300"
        ...
        {sessionMode === 'recording' && (hasMicrophone || isRecording) &&
          status !== RecordingStatus.PROCESSING_TRANSCRIPTS &&
          status !== RecordingStatus.SAVING && (
