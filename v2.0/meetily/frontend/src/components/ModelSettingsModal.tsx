...
export interface ModelConfig {
  provider: 'ollama' | 'groq' | 'claude' | 'openai' | 'deepseek' | 'openrouter' | 'builtin-ai' | 'custom-openai';
  model: string;
  whisperModel: string;
  apiKey?: string | null;
...
// deepseek-flash is DeepSeek's V4.1-Flash model (api-docs.deepseek.com).
...
export function ModelSettingsModal({
  ...
  const handleSave = async () => {
    ...
    // Never log the full config: it carries the provider API key.
    console.log('ModelSettingsModal - model config saved:', { provider: updatedConfig.provider, model: updatedConfig.model, hasApiKey: Boolean(updatedConfig.apiKey) });
