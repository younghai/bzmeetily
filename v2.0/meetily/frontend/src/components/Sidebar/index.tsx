  ...
  useEffect(() => {
    const setupListener = async () => {
      ...
      const unlisten = await listen<ModelConfig>('model-config-updated', (event) => {
        console.log('Sidebar model-config-updated:', { provider: event.payload?.provider, model: event.payload?.model, hasApiKey: Boolean(event.payload?.apiKey) });
      ...
      <TooltipProvider>
        <div className="flex flex-col items-center space-y-4 mt-4 pt-12">
          ...
          <Tooltip>
            ...
            <TooltipContent side="right">
              <p>{isRecording ? "녹음 진행 중" : "새 세션 시작"}</p>
            </TooltipContent>
          </Tooltip>
