...
export function ConfigProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const loadAllApiKeys = async () => {
      try {
        const providers = ['claude', 'groq', 'openai', 'deepseek', 'openrouter'];
  ...
  useEffect(() => {
    const setupListener = async () => {
      ...
      const unlisten = await listen<ModelConfig>('model-config-updated', (event) => {
        console.log('[ConfigContext] model-config-updated:', { provider: event.payload?.provider, model: event.payload?.model, hasApiKey: Boolean(event.payload?.apiKey) });
  ...
  const modelOptions: Record<ModelConfig['provider'], string[]> = {
    ...
    openrouter: [],
    openai: ['gpt-5.4', 'gpt-5.4-mini', 'gpt-5.3', 'gpt-5.3-mini', 'gpt-5.1', 'gpt-5', 'gpt-4.1', 'gpt-4o'],
