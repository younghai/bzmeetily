...
export function TranscriptProvider({ children }: { children: ReactNode }) {
  ...
  const [isLiveInterpretation, setIsLiveInterpretation] = useState(false);
  ...
  const liveInterpretationRef = useRef(false);
  const provisionalLiveInterpretationRef = useRef(false);
  ...
  useEffect(() => {
    if (!recordingState.isRecording || !recordingState.isLiveInterpretation) return;
    provisionalLiveInterpretationRef.current = false;
    liveInterpretationRef.current = true;
    setIsLiveInterpretation(true);
    if (currentMeetingId !== null) setCurrentMeetingId(null);
    sessionStorage.removeItem('indexeddb_current_meeting_id');
  }, [currentMeetingId, recordingState.isLiveInterpretation, recordingState.isRecording]);
  ...
  useEffect(() => {
    ...
    let unlistenRecordingStarting: (() => void) | undefined;
    ...
    const setupRecordingListeners = async () => {
      try {
        ...
        unlistenRecordingStarting = await recordingService.onRecordingStarting((payload) => {
          provisionalLiveInterpretationRef.current = payload.live_interpretation === true;
        });

        // Listen for recording-started event
        unlistenRecordingStarted = await recordingService.onRecordingStarted(async (payload) => {
          try {
            const live = payload.live_interpretation === true;
            provisionalLiveInterpretationRef.current = false;
            liveInterpretationRef.current = live;
            setIsLiveInterpretation(live);
            if (live) {
        ...
        unlistenRecordingStopped = await recordingService.onRecordingStopped(async (payload) => {
          try {
            if (!liveInterpretationRef.current && currentMeetingId) {
            ...
            provisionalLiveInterpretationRef.current = false;
            liveInterpretationRef.current = false;
            setIsLiveInterpretation(false);
          }
        });
      } catch (error) {
    ...
    return () => {
      ...
      if (unlistenRecordingStarting) unlistenRecordingStarting();
  ...
  useEffect(() => {
    if (recordingState.isRecording || recordingState.status !== RecordingStatus.ERROR) return;
    provisionalLiveInterpretationRef.current = false;
    ...
    const setupListener = async () => {
      try {
        ...
        unlistenFn = await transcriptService.onTranscriptUpdate((update) => {
          ...
          if (!liveInterpretationRef.current && !provisionalLiveInterpretationRef.current && currentMeetingId) {
  ...
  useEffect(() => {
    const syncFromBackend = async () => {
      ...
      if (recordingState.isRecording && !recordingState.isLiveInterpretation && !isLiveInterpretation && transcripts.length === 0) {
        try {
    ...
    syncFromBackend();
  }, [recordingState.isLiveInterpretation, recordingState.isRecording, isLiveInterpretation]); // Run when recording state changes
