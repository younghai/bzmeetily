...
export function useModelConfiguration({ serverAddress }: UseModelConfigurationProps) {
  ...
  const handleSaveModelConfig = useCallback(async (updatedConfig?: ModelConfig) => {
    try {
      ...
      // Redacted: payload.apiKey must never reach the console.
      console.log('Saving model config:', { provider: payload.provider, model: payload.model, hasApiKey: Boolean(payload.apiKey) });
